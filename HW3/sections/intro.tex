...
\begin{remark}
    Note that the log-likelihood function is just the logarithm of the likelihood function. It is sometimes more convenient to work with
    since the logarithm of a product is the sum of the logarithms hence, it transforms products into sums. More formally, the log-likelihood function is given by
    $$\ell (\textbf{y}|\theta) := \operatorname{log}_{e}L(\textbf{y}|\theta) = \sum_{i=1}^{n} \operatorname{log}_{e} f_{Y}(y_i | \theta)$$
\end{remark}

For the discrete case, switch $f_{Y}$ with $p_{Y}$ in the above expression.